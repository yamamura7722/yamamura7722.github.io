...
function getViewportSizeAtDepth(camera, depth, aspect) {
    var viewportHeightAtDepth = 2 * depth * Math.tan(THREE.MathUtils.degToRad(0.5 * camera.fov));
    var viewportWidthAtDepth = viewportHeightAtDepth * aspect;
	const h2 = window.innerHeight / viewportHeightAtDepth
//	if(viewportWidthAtDepth * h2 < window.innerWidth) {
//		viewportHeightAtDepth *= window.innerWidth / h2 / viewportWidthAtDepth;
//		viewportWidthAtDepth = window.innerWidth / h2;
//	}
 ...
 */
function createCameraPlaneMesh(camera, depth, aspect, material) {
    if (camera.near > depth || depth > camera.far) {
        console.warn("Camera plane geometry will be clipped by the `camera`!");
    }
    const viewportSize = getViewportSizeAtDepth(camera, depth, aspect);
...
//const orbitCamera = new THREE.PerspectiveCamera(60,window.innerWidth / window.innerHeight,0.01,5000);
...
//let orbitTarget = orbitCamera.position.clone();
//orbitTarget.z -= 5;
...
inputFrameTexture.flipY = false;
inputFrameTexture.center = new THREE.Vector2(0.5, 0.5);
inputFrameTexture.rotation = Math.PI;
//inputFrameTexture.rotation = -1.5;
//inputFrameTexture.updateMatrix();
const inputFramesDepth = 500;
const inputFramesPlane = createCameraPlaneMesh(orbitCamera, inputFramesDepth, 4 / 3, new THREE.MeshBasicMaterial({ map: inputFrameTexture }));
  ...
  if (pose2DLandmarks && pose3DLandmarks) {
    ...
    rigPosition(
      "Hips",
      {
        ...
        y: riggedPose.Hips.position.y + 0.8, // Add a bit of height
